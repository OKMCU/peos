...
static void st_timer_event_kernel ( void *p_arg )
{
    ...
    u16tmp = (st_uint16_t)( (st_uint32_t)p_arg );
    task_id = (st_uint8_t)HI_UINT16( u16tmp );
    event_id = (st_int8_t)LO_UINT16( u16tmp );
    st_task_set_event( task_id, event_id );
}

static ST_TIMER_t *st_timer_event_find( st_uint8_t task_id, st_int8_t event_id )
{
    ST_TIMER_t *p_timer_match;
    void *p_arg;
...
static void    *st_timer_cback_create ( void ( *p_fxn )( void * ), void *p_arg, st_uint32_t tick )
{
...
st_timer_event_find( st_uint8_t task_id, st_int8_t event_id )
{
#if (ST_TIMER_MAX >= UINT8_MAX)
...
st_err_t st_timer_create ( st_uint8_t task_id, st_int8_t event_id, st_uint32_t tick )
{
    ...
    else
    {
        ...
        p_timer_match = (void *)st_timer_cback_create( st_timer_event_kernel, (void *)BUILD_UINT16( event_id, task_id ), tick);
        if( p_timer_match == NULL )
            return ST_ERR_NOMEM;
    }
#else
